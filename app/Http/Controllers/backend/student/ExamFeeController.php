...
class ExamFeeController extends Controller
{
  ...
  public function view(){
    ...
    return view("backend.student.student-fees.view-exam-fee",compact("year","class","exam_type"));
}

public function get_student_exam_fee(Request $request){
   ...
   foreach ($alldata as $key => $value) {
       $monthlyFee= FeesAmount::where("fee_category_id","3")->where("class_id",$value->class_id)->first();
       ...
       $html[$key]["tdsourch"] .= '<td><a href="'.route('get_pdf_exam_fee',$value->student_id).'?class_id='.$value->class_id.'&student_id='.$value->student_id.'&month='.$request->month_id.'"  class="btn btn-info btn-sm"><i class="fa fa-eye"></i></a></td>';
...
public function get_pdf_exam_fee(Request $request){
    $class_id=$request->class_id;
    $student_id= $request->student_id;
    $exam_type = $request->month;
   $exam_type_name= ExamType::where("id",$exam_type)->first();



    $alldata = AssignStudent::with("discount")->where("student_id",$student_id)->where("class_id",$class_id)->first();

    $pdf= PDF::loadView("backend.student.student-fees.exam-paysleep",compact("alldata","exam_type_name"));
