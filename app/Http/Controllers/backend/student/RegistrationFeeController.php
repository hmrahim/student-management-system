...
class RegistrationFeeController extends Controller
{
  ...
  public function view(){
    ...
    return view("backend.student.student-fees.view-regi-fee",compact("year","class",));
}

public function get_student_regi_fee(Request $request){
   ...
   $html["thsourch"] .= '<th>Registration Fee</th>';
   $html["thsourch"] .= '<th>DIscount</th>';
   $html["thsourch"] .= '<th>Fee(This student)</th>';
   $html["thsourch"] .= '<th>Action</th>';

   foreach ($alldata as $key => $value) {
       $registrationfee= FeesAmount::where("fee_category_id","1")->where("class_id",$value->class_id)->first();
       ...
       $html[$key]["tdsourch"] .= '<td>'.$registrationfee->fees_amount.'tk </td>';
       ...
       $html[$key]["tdsourch"] .= '<td><a href="'.route('get_pdf',$value->student_id).'?class_id='.$value->class_id.'&student_id='.$value->student_id.'"  class="btn btn-info btn-sm"><i class="fa fa-eye"></i></a></td>';
...
public function get_pdf(Request $request){
    ...
    $pdf= PDF::loadView("backend.student.student-fees.paysleep",compact("alldata"));
    $pdf->SetProtection(["copy","print"],"", "");
    return $pdf->stream( $alldata->user->id_no.".pdf");
